...
public class Http {
    ...
    public static void ByteToFile(byte[] bytes, HttpServletResponse response)throws Exception{
        ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
        BufferedImage bi1 = ImageIO.read(bais);
        try {
//            File w2 = new File("W:\\img\\00000000003.jpg");//可以是jpg,png,gif格式
            ImageIO.write(bi1, "jpg", response.getOutputStream());//不管输出什么格式图片，此处不需改动
        } catch (IOException e) {
            e.printStackTrace();
        }finally{
            bais.close();
